...
static esp_err_t esp_lcd_new_panel_ili9341(const esp_lcd_panel_io_handle_t io, const esp_lcd_panel_dev_config_t *panel_dev_config, esp_lcd_panel_handle_t *ret_panel)
{
    ...
    *ret_panel = &(ili9341->base);
    ...
    esp3d_log("LCD panel create success, version: %d.%d.%d", ESP_LCD_ILI9341_VER_MAJOR, ESP_LCD_ILI9341_VER_MINOR, ESP_LCD_ILI9341_VER_PATCH);
...
static esp_err_t panel_ili9341_init(esp_lcd_panel_t *panel)
{
    ...
    for (int i = 0; i < sizeof(ili9341_init_commands) / sizeof(ili9341_init_cmd_t); i++) {
        //esp3d_log("Command %d: 0x%02X, %d bytes", i, ili9341_init_commands[i].cmd, ili9341_init_commands[i].data_bytes);
...
static esp_err_t panel_ili9341_draw_bitmap(esp_lcd_panel_t *panel, int x_start, int y_start, int x_end, int y_end, const void *color_data)
{
    ...
    ret = esp_lcd_panel_io_tx_param(io, LCD_CMD_CASET, (uint8_t[]) {
        (x_start >> 8) & 0xFF,
        x_start & 0xFF,
        ((x_end - 1) >> 8) & 0xFF,
        (x_end - 1) & 0xFF,
...
static esp_err_t panel_ili9341_mirror(esp_lcd_panel_t *panel, bool mirror_x, bool mirror_y)
{
    ...
    esp_err_t ret = esp_lcd_panel_io_tx_param(io, LCD_CMD_MADCTL, (uint8_t[]) {
        ili9341->madctl_val
    }, 1);
    if (ret != ESP_OK) {
...
esp_err_t ili9341_spi_configure(const spi_ili9341_config_t *config)
{
    ...
    esp3d_log("    Swap color bytes: %s", spiIli9341Config.interface.swap_color_bytes ? "Yes" : "No");
    ...
    if (spiIli9341Config.spi_bus.is_master) {
        ...
        esp3d_log("SPI bus initialized successfully on host %s", spi_names[spiIli9341Config.spi_bus.host]);
    } else {
    ...
    esp3d_log("    Color transfer callback: %s", spiIli9341Config.lvgl.enable_callbacks ? "Enabled" : "Disabled");
    ...
    esp3d_log("    RGB element order: %s", spiIli9341Config.interface.swap_color_bytes ? "BGR" : "RGB");
