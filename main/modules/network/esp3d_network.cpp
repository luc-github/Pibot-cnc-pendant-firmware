...
static void
wifi_ap_event_handler(void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data)
{
    if (event_id == WIFI_EVENT_AP_STACONNECTED)
    {
#if ESP3D_TFT_LOG && ESP3D_TFT_LOG == ESP3D_TFT_LOG_LEVEL_ALL
        wifi_event_ap_staconnected_t *event = (wifi_event_ap_staconnected_t *)event_data;
        esp3d_log_d("station " MACSTR " join, AID=%d", MAC2STR(event->mac), event->aid);
#else
    ...
    else if (event_id == WIFI_EVENT_AP_STADISCONNECTED)
    {
        ...
        esp3d_log_d("station " MACSTR " leave, AID=%d", MAC2STR(event->mac), event->aid);
#else
        // TODO: What need to be done here ?
#endif  // ESP3D_TFT_LOG
    }
...
static void
wifi_sta_event_handler(void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data)
{
    ...
    else if (event_base == IP_EVENT && event_id == IP_EVENT_STA_GOT_IP)
    {
        ...
        esp3d_log_d("got ip:" IPSTR, IP2STR(&event->ip_info.ip));
#else
        // TODO: What need to be done here ?
#endif  // ESP3D_TFT_LOG
        s_retry_num = 0;
    ...
    {
        esp3d_log_d("Client IP was lost");
        esp3dTftValues.set_string_value(ESP3DValuesIndex::status_bar_label, "0.0.0.0");
    ...
    switch (_current_radio_mode)
    {
        case ESP3DRadioMode::wifi_sta:
            if (_wifiStaPtr)
            {
                ...
                {
                    if (ESP_OK
                        == esp_netif_get_dns_info(_wifiStaPtr,
                                                  ESP_NETIF_DNS_MAIN,
                                                  &(ipInfo->dns_info)))
                    {
                        esp3dTftValues.set_string_value(ESP3DValuesIndex::network_status, "+");
        ...
        case ESP3DRadioMode::wifi_ap:
            if (_wifiApPtr)
            {
                ...
                {
                    if (ESP_OK
                        == esp_netif_get_dns_info(_wifiApPtr,
                                                  ESP_NETIF_DNS_MAIN,
                                                  &(ipInfo->dns_info)))
                    {
                        return true;
...
bool ESP3DNetwork::begin()
{
    ...
    esp3d_log_d("Free mem %ld", esp_get_minimum_free_heap_size());
    esp3dTftValues.set_string_value(ESP3DValuesIndex::network_mode, LV_SYMBOL_WIFI);
    esp3dTftValues.set_string_value(ESP3DValuesIndex::network_status, "x");
    if (!bootDone)
    {
        bootDone         = true;
        uint8_t bootMode = esp3dTftsettings.readByte(ESP3DSettingIndex::esp3d_radio_boot_mode);

        if (bootMode == static_cast<uint8_t>(ESP3DRadioMode::off))
        {
            esp3d_log_d("Radio is off at boot time");
            esp3dTftValues.set_string_value(ESP3DValuesIndex::network_status, ".");
            _started = true;
            return true;
        }
    ...
    uint8_t radioMode = esp3dTftsettings.readByte(ESP3DSettingIndex::esp3d_radio_mode);
...
bool ESP3DNetwork::setModeAsync(ESP3DRadioMode mode)
{
    ...
    esp3d_log_d("Enabling async mode to %d", (uint8_t)_async_radio_mode);
    return true;
}
...
void ESP3DNetwork::handle()
{
        ...
        && (xEventGroupGetBits(_s_wifi_event_group) & WIFI_STA_LOST_IP))
    {
        ...
        esp3d_log_d("Force restart wifi station");
    ...
    {
        esp3d_log_d("Asking for change to %d", (uint8_t)_async_radio_mode);
...
const char *ESP3DNetwork::getModeStr(ESP3DRadioMode mode)
{
    switch (mode)
    {
        ...
        case ESP3DRadioMode::bluetooth_ble:
...
bool ESP3DNetwork::startStaMode()
{
    ...
    if (!_s_wifi_event_group)
    {
        esp3d_log_d("Cannot create event group");
        return false;
    }
    ...
    esp3d_log_d("Free mem %ld", esp_get_minimum_free_heap_size());
    esp3d_log_d("Create default sta service");
    ...
    esp3d_log_d("Register wifi handler");
    ...
    ESP_ERROR_CHECK(esp_event_handler_instance_register(IP_EVENT,
                                                        ...
                                                        NULL,
                                                        &instance_got_ip));
    esp3d_log_d("Configure STA settings");
    esp3dTftsettings.readString(ESP3DSettingIndex::esp3d_sta_ssid, ssid_str, 33);
    esp3dTftsettings.readString(ESP3DSettingIndex::esp3d_sta_password, ssid_pwd_str, 65);
    uint8_t ipMode = esp3dTftsettings.readByte(ESP3DSettingIndex::esp3d_sta_ip_mode);
    esp3d_log_d("Got  STA settings: SSID:<%s> password:<%s> ip mode :<%s>",
              ssid_str,
              ssid_pwd_str,
              ipMode == static_cast<uint8_t>(ESP3DIpMode::staticIp) ? "Static" : "DHCP");
    ...
    {
        _useStaticIp = true;
        esp3d_log_d("Set IP static mode");
        esp_netif_dhcpc_stop(_wifiStaPtr);
        uint32_t ip_int  = esp3dTftsettings.readUint32(ESP3DSettingIndex::esp3d_sta_ip_static);
        uint32_t gw_int  = esp3dTftsettings.readUint32(ESP3DSettingIndex::esp3d_sta_gw_static);
        uint32_t msk_int = esp3dTftsettings.readUint32(ESP3DSettingIndex::esp3d_sta_mask_static);
        uint32_t dns_int = esp3dTftsettings.readUint32(ESP3DSettingIndex::esp3d_sta_dns_static);
    ...
    stmp += ssid_str;
    stmp += "\n";
    esp3dCommands.dispatch(stmp.c_str(),
                           ESP3DClientType::all_clients,
                           requestId,
    ...
    {
        esp3d_log_d("connected to ap SSID: %s, password: %s", ssid_str, ssid_pwd_str);
        esp_netif_ip_info_t ip_info;
    ...
    {
        esp3d_log_e("Failed to connect to SSID: %s, password: %s", ssid_str, ssid_pwd_str);
    }
    else
    {
    ...
    if (!connected)
    {
        ...
        stmp += ssid_str;
        stmp += " failed";
        esp3dTftValues.set_string_value(ESP3DValuesIndex::status_bar_label, stmp.c_str());
    }
...
bool ESP3DNetwork::startApMode(bool configMode, bool limited)
{
    ...
    esp3d_log_d("Init AP Mode");
    esp3d_log_d("Free mem %ld", esp_get_minimum_free_heap_size());

    esp3d_log_d("Create default ap service");
    wifi_init_config_t cfg = WIFI_INIT_CONFIG_DEFAULT();
    // init the wifi
    ESP_ERROR_CHECK(esp_wifi_init(&cfg));
    esp3d_log_d("Free mem %ld", esp_get_minimum_free_heap_size());
    ...
    esp3d_log_d("Configure AP settings");
    ...
    uint8_t channel = esp3dTftsettings.readByte(ESP3DSettingIndex::esp3d_ap_channel);
    uint32_t ip_int = esp3dTftsettings.readUint32(ESP3DSettingIndex::esp3d_ap_ip_static);
    esp3dTftsettings.readString(ESP3DSettingIndex::esp3d_ap_password, ssid_pwd_str, 65);
    esp3d_log_d("Got  AP settings: SSID:<%s> password:<%s> channel:<%d>",
              ssid_str,
              ssid_pwd_str,
              channel);
    wifi_config_t wifi_config;
    ...
    esp3d_log_d("Setup IP and DHCP range");
    ...
    esp3d_log_d("set AP mode");
    ESP_ERROR_CHECK(esp_wifi_set_mode(WIFI_MODE_AP));
    esp3d_log_d("Set configuration");
    ESP_ERROR_CHECK(esp_wifi_set_config(WIFI_IF_AP, &wifi_config));
    esp3d_log_d("Start wifi");
    ESP_ERROR_CHECK(esp_wifi_start());
    // Set hostname
    const ESP3DSettingDescription *settingPtr =
        esp3dTftsettings.getSettingPtr(ESP3DSettingIndex::esp3d_hostname);
    ...
    {
#if ESP3D_TFT_LOG
        esp3d_log_d("wifi_init_softap done. - SSID:%s - channel:%d", ssid_str, channel);
        esp3d_log_d("AP IP: " IPSTR, IP2STR(&ip_info.ip));
        esp3d_log_d("AP GW: " IPSTR, IP2STR(&ip_info.gw));
        esp3d_log_d("AP NETMASK: " IPSTR, IP2STR(&ip_info.netmask));
#endif  // ESP3D_TFT_LOG
        stmp = "Access Point ";
        if (configMode)
        {
...
{
    esp3d_log_d("Stop No Radio Mode");
    return true;
}
#if ESP3D_BT_FEATURE
bool ESP3DNetwork::startBtSerialMode()
{
    esp3d_log_d("Init BT Serial Mode");
    _current_radio_mode = ESP3DRadioMode::bluetooth_serial;
    btSerialClient.begin();
    return false;
}
bool ESP3DNetwork::startBtBleMode()
{
    esp3d_log_d("Init BT BLE Mode");
    _current_radio_mode = ESP3DRadioMode::bluetooth_ble;
    //FIXME: Add code to start BT BLE mode
    return false;
}
bool ESP3DNetwork::stopBtBleMode()
{
    esp3d_log_d("Stop BT BLE Mode");
    //FIXME: Add code to stop BT BLE mode
    return false;
}
bool ESP3DNetwork::stopBtSerialMode()
{
    esp3d_log_d("Stop BT Serial Mode");
    btSerialClient.end();
    return false;
}
#endif  // ESP3D_BT_FEATURE
...
bool ESP3DNetwork::stopStaMode()
{
    ...
    if (_s_wifi_event_group)
    {
        esp3d_log_d("Clear xEventGroupBits");
        xEventGroupClearBits(_s_wifi_event_group, WIFI_CONNECTED_BIT);
        xEventGroupClearBits(_s_wifi_event_group, WIFI_FAIL_BIT);
        xEventGroupClearBits(_s_wifi_event_group, WIFI_STA_LOST_IP);
...
bool ESP3DNetwork::stopApMode()
{
    ...
    if (_wifiApPtr)
    {
        esp3d_log_d("Stop DHCP");
        esp_netif_dhcps_stop(_wifiApPtr);
        esp3d_log_d("Destroy default wifi AP");
        ESP_ERROR_CHECK(esp_wifi_deinit());
...
{
    esp3d_log_d("Current mode is %d, and ask for %d",
              static_cast<uint8_t>(_current_radio_mode),
              static_cast<uint8_t>(mode));
    /* if (mode == _current_radio_mode && !restart) {
       esp3d_log_d("Current mode and new mode are identical so cancel");
       return true;
     }*/
    esp3dTftValues.set_string_value(ESP3DValuesIndex::network_status, "x");
    ...
    switch (mode)
    {
        ...
        case ESP3DRadioMode::wifi_ap:
            startApMode();
            esp3dTftValues.set_string_value(ESP3DValuesIndex::network_mode, LV_SYMBOL_ACCESS_POINT);
            break;
        case ESP3DRadioMode::wifi_ap_config:
            startConfigMode();
            esp3dTftValues.set_string_value(ESP3DValuesIndex::network_mode, LV_SYMBOL_ACCESS_POINT);
        ...
        case ESP3DRadioMode::bluetooth_serial:
            startBtSerialMode();
            esp3dTftValues.set_string_value(ESP3DValuesIndex::network_mode, LV_SYMBOL_BLUETOOTH);
            break;
        case ESP3DRadioMode::bluetooth_ble:
            startBtBleMode();
            esp3dTftValues.set_string_value(ESP3DValuesIndex::network_mode, LV_SYMBOL_BLUETOOTH);
