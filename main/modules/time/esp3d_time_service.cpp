...
bool TimeService::isInternetTime(bool readfromsettings)
{
    if (readfromsettings)
    {
        _is_internet_time = esp3dTftsettings.readByte(ESP3DSettingIndex::esp3d_use_internet_time);
        esp3d_log("Internet time is %s", _is_internet_time ? "enabled" : "disabled");
    }
    return _is_internet_time;
}

const char *TimeService::getTimeZone()
{
    return _time_zone.c_str();
}

bool TimeService::setTimeZone(const char *stime)
{
    if (esp3dTftsettings.isValidStringSetting(stime, ESP3DSettingIndex::esp3d_timezone))
    {
        _time_zone = stime;
        return esp3dTftsettings.writeString(ESP3DSettingIndex::esp3d_timezone, _time_zone.c_str());
    }
    return false;
}
...
bool TimeService::begin()
{
    ...
    // server 1
    std::string stmp = esp3dTftsettings.readString(ESP3DSettingIndex::esp3d_time_server1,
                                                   out_str,
                                                   SIZE_OF_SERVER_URL);
    if (stmp.length() > 0)
    {
...
bool TimeService::updateTimeZone(bool fromsettings)
{
    ...
    _time_zone =
        esp3dTftsettings.readString(ESP3DSettingIndex::esp3d_timezone, out_str, SIZE_OF_TIMEZONE);
    if (!esp3dTftsettings.isValidStringSetting(_time_zone.c_str(),
