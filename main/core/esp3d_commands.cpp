...
#if ESP3D_TFT_LOG
static const char *esp3dclientstr[] = {"no_client",
                                       "serial",
                                       "usb_serial",
                                       "stream",
                                       "telnet",
...
#define GETCLIENTSTR(id)                                                                      \
    static_cast<int8_t>(id) >= 0                                                              \
            && static_cast<uint8_t>(id) <= static_cast<uint8_t>(ESP3DClientType::all_clients) \
...
#define GETMSGTYPESTR(id)                                                                 \
    static_cast<int8_t>(id) >= 0                                                          \
            && static_cast<uint8_t>(id) <= static_cast<uint8_t>(ESP3DMessageType::unique) \
...
bool ESP3DCommands::is_esp_command(uint8_t *sbuf, size_t len)
{
    ...
    if ((char(sbuf[0]) == '[') && (char(sbuf[1]) == 'E') && (char(sbuf[2]) == 'S')
        && (char(sbuf[3]) == 'P')
        && ((char(sbuf[4]) == ']') || (char(sbuf[5]) == ']') || (char(sbuf[6]) == ']')
            || (char(sbuf[7]) == ']')))
    {
        return true;
    }
    ...
    if (len >= 14)
    {
        if ((char(sbuf[0]) == 'e') && (char(sbuf[1]) == 'c') && (char(sbuf[2]) == 'h')
            && (char(sbuf[3]) == 'o') && (char(sbuf[4]) == ':') && (char(sbuf[5]) == ' ')
            && (char(sbuf[6]) == '[') && (char(sbuf[7]) == 'E') && (char(sbuf[8]) == 'S')
            && (char(sbuf[9]) == 'P')
            && ((char(sbuf[4]) == ']') || (char(sbuf[5]) == ']') || (char(sbuf[6]) == ']')
                || (char(sbuf[7]) == ']')))
        {
...
void ESP3DCommands::process(ESP3DMessage *msg)
{
    ...
    if (is_esp_command(msg->data, msg->size))
    {
        ...
        for (uint i = 0; i < msg->size && espcmdpos == 0; i++)
        {
            ...
            else if (char(msg->data[i]) == '[')
            {  // end command flag
                startcmd = true;
            }
            else if (startcmd && !endcmd
                     && std::isdigit(static_cast<unsigned char>(char(msg->data[i]))))
            {  // command id
                if (cmdId != 0)
                {
                    cmdId = (cmdId * 10);
    ...
    {
        esp3d_log("Dispatch command, len %d, to %d", msg->size, static_cast<uint8_t>(msg->target));
                                    ...
                                    bool isFirst)
{
    ...
    const ESP3DSettingDescription *elementSetting = esp3dTftsettings.getSettingPtr(index);
    if (!elementSetting)
    {
        return false;
    }
...
bool ESP3DCommands::dispatchAuthenticationError(ESP3DMessage *msg, uint cmdid, bool json)
{
    std::string tmpstr;
    if (!msg)
    {
...
bool ESP3DCommands::dispatchAnswer(ESP3DMessage *msg,
                                   uint cmdid,
                                   bool json,
                                   bool hasError,
...
bool ESP3DCommands::dispatchKeyValue(bool json,
                                     const char *key,
                                     const char *value,
                                     ESP3DClientType target,
                                     ESP3DRequest requestId,
...
bool ESP3DCommands::dispatchIdValue(bool json,
                                    const char *Id,
                                    const char *value,
                                    ESP3DClientType target,
                                    ESP3DRequest requestId,
...
bool ESP3DCommands::dispatch(const char *sbuf,
                             ESP3DClientType target,
                             ESP3DRequest requestId,
...
{
    bool sendOk = true;
    esp3d_log("Dispatch message origin %d(%s) to client %d(%s) , size: %d,  type: "
              "%d(%s)",
              static_cast<uint8_t>(msg->origin),
              GETCLIENTSTR(msg->origin),
              static_cast<uint8_t>(msg->target),
              GETCLIENTSTR(msg->target),
              msg->size,
              static_cast<uint8_t>(msg->type),
              GETMSGTYPESTR(msg->type));
    if (!msg)
    {
        esp3d_log_e("no msg");
        return false;
    ...
    switch (msg->target)
    {
        ...
        case ESP3DClientType::all_clients:
            ...
            if (msg->origin != ESP3DClientType::stream && msg->origin != ESP3DClientType::system)
            {
            ...
            if (msg->origin != ESP3DClientType::rendering && msg->origin != ESP3DClientType::command
        ...
        // Send pending if any or cancel message is no client did handle it
            ...
            break;
        default:
            esp3d_log_e("No valid target specified %d", static_cast<uint8_t>(msg->target));
            sendOk = false;
    }
    // clear message
    if (!sendOk)
...
const char *ESP3DCommands::get_param(ESP3DMessage *msg, uint start, const char *label, bool *found)
{
    if (!msg)
    {
        return "";
...
const char *
ESP3DCommands::get_param(const char *data, uint size, uint start, const char *label, bool *found)
{
    int startPos  = -1;
    uint lenLabel = strlen(label);
...
void ESP3DCommands::execute_internal_command(int cmd, int cmd_params_pos, ESP3DMessage *msg)
{
    ...
    if (!pwd.empty())
    {  // adjust authentication level according
        ...
        if (msg->authentication_level != lvl)
        {
            ...
            if (!esp3dAuthenthicationService.updateRecord(msg->request_id.id, msg->origin, lvl))
            {
    ...
    switch (cmd)
    {
        ...
        case 902:
            ...
            break;
#endif  // #if ESP3D_USB_SERIAL_FEATURE
#if ESP3D_BT_FEATURE || ESP3D_USB_SERIAL_FEATURE
        case 950:
...
ESP3DClientType ESP3DCommands::getOutputClient(bool fromSettings)
{
    if (fromSettings)
    {
        ...
        uint8_t value = esp3dTftsettings.readByte(ESP3DSettingIndex::esp3d_output_client);
...
#if ESP3D_BT_FEATURE
            case ESP3DClientType::bt_serial:
                _output_client = ESP3DClientType::bt_serial;
                break;
            case ESP3DClientType::bt_ble:
                _output_client = ESP3DClientType::bt_ble;
                break;
#endif  // #if ESP3D_BT_FEATURE
